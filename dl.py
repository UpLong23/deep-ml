...
def batch_normalization(
    ...
    mean_per_channel = X.mean(axis=(0,2,3)).reshape(1,2,1,1)
    var_per_channel = X.var(axis=(0,2,3)).reshape(1,2,1,1)
    if (running_mean is None):
        running_mean = np.zeros((1,2,1,1))
    if (running_var is None):
        running_var = np.ones((1,2,1,1))
...
batch_normalization(X = np.random.randn(2, 2, 2, 2),
                    gamma = np.ones((1, 2, 1, 1)),
                    beta = np.zeros((1, 2, 1, 1)),
                    training = True)[0]
...
_, rm, rv = batch_normalization(X, gamma, beta, running_mean, running_var, momentum=0.1, training=True) 
print(np.round(rm, 5)) 
print(np.round(rv, 5))
# %%
